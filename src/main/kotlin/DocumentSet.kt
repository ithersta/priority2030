...
//        field("ACCOUNT_NUMBER", get<BankInfo>().settlementAccountNumber)
    ...
    when (get<OrganizationType>()) {
        OrganizationType.IP -> document("/Шаблон договора дл ИП С МЕТКАМИ.docx") {
            ...
            field("ENPREPRENEUR_INIC", get<IpInfo>().surnameAfterInitials) // Фамилия и  инициалы предпринимателя
            field("ENPREPRENEUR_INIC_F", get<IpInfo>().initialsAfterSurname) // Инициалы и фамилия предпринимателя
...
//            field("CONTRAGENT_SHORT_NAME") //Наименование контрагента сокращённо ИЗ ИНН
            field("GENERAL_MANAGER", get<CompanyInfo>().fullNameOfHolder) //Генеральный директор ФИО в именительном падеже полностью
...
//            field("SUMM_COP_NUMB") //Сумма цифрами  в копейках
            ...
            field("GENERAL_MANAGER_INIC", get<CompanyInfo>().initialsAfterSurname) //Инициалы и фамилия генерального директора
        }
    }
}
